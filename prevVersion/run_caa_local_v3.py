...
class ExperimentConfig:
    ...
    alpha_values: List[float] = field(default_factory=lambda: [0.0, 1.0, 2.0, 3.0])
    ...
    results_csv_path: str = "steering_results_v3.csv"
    seed: int = 42
...
for alpha in config.alpha_values:
    ...
    for i, prompt in enumerate(tqdm(test_prompts)):
        ...
        results.append({
            ...
            "generated_code": generated[:500], # Crucial for qualitative verification
        })
        torch.cuda.empty_cache()
        gc.collect()
...
print(f"\n✓ Full results with generated code saved to {config.results_csv_path}")
...
for a, rate in zip(vuln_rates.index, vuln_rates.values):
    ax.annotate(f'{rate:.1f}%', xy=(a, rate), xytext=(0, 10), textcoords='offset points', 
...
print(f"\n{'='*70}")
print(f"SAMPLE QUALITATIVE OUTPUT: STEERED vs. UNSTEERED")
print(f"{'='*70}")

max_alpha = max(config.alpha_values)

# Find a prompt that was vulnerable at baseline
vuln_baseline_idx = df[(df['alpha'] == 0.0) & (df['is_vulnerable'] == True)]['prompt_index'].tolist()
if len(vuln_baseline_idx) > 0:
    sample_idx = vuln_baseline_idx[0]
    prompt_text = df[(df['alpha'] == 0.0) & (df['prompt_index'] == sample_idx)]['prompt'].iloc[0]
    baseline_code = df[(df['alpha'] == 0.0) & (df['prompt_index'] == sample_idx)]['generated_code'].iloc[0]
    steered_code = df[(df['alpha'] == max_alpha) & (df['prompt_index'] == sample_idx)]['generated_code'].iloc[0]
    baseline_vulns = df[(df['alpha'] == 0.0) & (df['prompt_index'] == sample_idx)]['vulnerability_classes'].iloc[0]
    steered_vulns = df[(df['alpha'] == max_alpha) & (df['prompt_index'] == sample_idx)]['vulnerability_classes'].iloc[0]

    print(f"\n➤ PROMPT: {prompt_text}...")
    print(f"\n❌ UNSTEERED (α=0.0) [Findings: {baseline_vulns}]\n{'-'*50}")
    for line in baseline_code[:300].split('\n'):
        print(f"  {line}")
    
    print(f"\n✅ STEERED (α={max_alpha}) [Findings: {steered_vulns}]\n{'-'*50}")
    for line in steered_code[:300].split('\n'):
        print(f"  {line}")
else:
    print("No vulnerable baseline outputs found to compare.")
